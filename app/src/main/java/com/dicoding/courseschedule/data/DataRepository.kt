...
class DataRepository(private val dao: CourseDao) {
    ...
    fun getNearestSchedule(queryType: QueryType) : LiveData<Course?> = dao.getNearestSchedule(nearestQuery(queryType))


    fun getAllCourse(sortType: SortType): LiveData<PagedList<Course>> = LivePagedListBuilder(dao.getAll(sortedQuery(sortType)), PAGE_SIZE).build()
